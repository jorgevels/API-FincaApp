...
const { ventasMock } = require('./ventas');
 ...
 */
getAllStub.withArgs('ventas').resolves(ventasMock);

const createStub = sinon.stub().resolves(ventasMock[0].id);

const updateStub = sinon
  .stub()
  .withArgs('ventas', ID)
  .resolves(ventasMock[0].id);
